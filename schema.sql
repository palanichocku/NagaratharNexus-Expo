...
CREATE OR REPLACE FUNCTION "public"."search_profile_cards_v1"("p_query" "text", "p_min_age" integer, "p_max_age" integer, "p_min_height" integer, "p_max_height" integer, "p_countries" "text"[], "p_marital_statuses" "text"[], "p_interests" "text"[], "p_education" "text"[], "p_exclude_kovil_pirivu" "text"[], "p_page_size" integer, "p_cursor_updated_at" timestamp with time zone, "p_cursor_id" "uuid", "p_exclude_user_id" "uuid", "p_forced_gender" "text") RETURNS TABLE("id" "uuid", "full_name" "text", "age" integer, "gender" "text", "resident_country" "text", "resident_status" "text", "current_state" "text", "current_city" "text", "profession" "text", "height_inches" integer, "kovil" "text", "pirivu" "text", "native_place" "text", "profile_photo_url" "text", "updated_at" timestamp with time zone)
    LANGUAGE "sql" STABLE
    AS $$
  select
    p.id,
  ...
  where
    ...
    -- age/height bounds
    and (p.age is not null and p.age between p_min_age and p_max_age)
    and (p.height_inches is not null and p.height_inches between p_min_height and p_max_height)

    -- query (keep your existing query logic; this is a basic example)
    and (
      p_query is null
      or p.full_name ilike ('%' || p_query || '%')
      or p.profession ilike ('%' || p_query || '%')
      or p.current_city ilike ('%' || p_query || '%')
      or p.current_state ilike ('%' || p_query || '%')
    )
    ...
    -- forced gender (USER search)
    and (p_forced_gender is null or p.gender = p_forced_gender)

    -- ✅ NEW: interests (any overlap)
    and (p_interests is null or p.interests && p_interests)

    -- ✅ NEW: education (any edu level matches)
    and (
      p_education is null
      or exists (
        select 1
        from jsonb_array_elements(coalesce(p.education_history, '[]'::jsonb)) e
        where (e->>'level') = any(p_education)
      )
    )

    -- exclude kovil/pirivu pairs (keep your existing logic; placeholder)
    and (
      p_exclude_kovil_pirivu is null
      or not (
        (p.kovil || '||' || coalesce(p.pirivu,''))
        = any(p_exclude_kovil_pirivu)
        or (p.kovil || '||*') = any(p_exclude_kovil_pirivu)
      )
    )

    -- keyset pagination (keep your exact logic; placeholder)
    and (
